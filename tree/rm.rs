...
use plib::{
    walkdir::{self, traverse_directory},
    PROJECT_NAME,
};
use std::io;
...
fn is_file_writable(md: Option<&walkdir::Metadata>) -> bool {
    match md {
        Some(md) => {
            let stat = &md.0;
            ...
            let same_user = stat.st_uid == uid;
            let same_group = stat.st_gid == gid;
...
// is not required.
fn should_remove_file<F>(cfg: &RmConfig, metadata: &walkdir::Metadata, filename_fn: F) -> bool
where
    F: Fn() -> String,
{
    let writable = is_file_writable(Some(metadata));
    if ask_for_prompt(cfg, writable) {
        ...
        let prompt = match file_type {
            ...
            walkdir::FileType::Directory => unreachable!(), // Handled in the caller
        };
...
fn rm_directory(cfg: &RmConfig, filepath: &Path) -> io::Result<bool> {
    ...
    let success = traverse_directory(
        ...
        |entry| {
            ...
            if md.file_type() == walkdir::FileType::Directory {
        ...
        |entry, error| match error.kind() {
            ...
            walkdir::ErrorKind::Open | walkdir::ErrorKind::DirNotSearchable => {
                eprintln!(
                    "rm: {}",
                    gettext!(
                        "cannot remove '{}': {}",
            ...
            walkdir::ErrorKind::ReadLink => unreachable!(), // rm doesn't follow symlinks
        },
...
fn rm_file(cfg: &RmConfig, filepath: &Path) -> io::Result<bool> {
    ...
    let metadata = walkdir::Metadata::new(libc::AT_FDCWD, filename_cstr.as_ptr(), false)?;
