...
- [ ] **#C60 — `__attribute__((section(...)))` attaches to the declaration, not the declarator.** **Open, Trivial.** `int a, b __attribute__((section(".s")));` gives it to `a`. Follows the pending-alignment attribute, which has the same shape and the same limitation.
