...
| Darwin: an over-aligned variadic aggregate | clang disagrees with itself here, so there is no behaviour to match. Its *caller* lowers the aggregate through LLVM's Darwin vararg convention, which stacks it a legalized element at a time in eight-byte granules and never reads the `aligned` attribute; its `va_arg` rounds the cursor up to the type's full alignment. c17 follows `va_arg`, and its caller realigns the outgoing area so the argument really is that aligned. A c17 caller therefore reaches a clang callee, and a clang caller does not reach a c17 callee -- the second direction of `codegen_over_aligned_argument_area`'s host-compiler cross-check fails on macOS and cannot be made to pass while the first one does |
