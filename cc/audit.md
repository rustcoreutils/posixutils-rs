...
- [ ] **#C18 — Struct layout is wrong when bitfields mix with other members.** **Open, Major.** **[probed]** `struct D { char x; unsigned a:1; char y; }` is 4 bytes under gcc and **12** under c17; `struct C { int a:4; unsigned b:4; signed char c:3; }` is 4 under gcc and 8 under c17. Both are ABI-visible: a struct passed between a c17 object and a gcc one is laid out differently, so this is worse than a size mismatch.

  **Diagnosed** (`compute_struct_layout`, `cc/types.rs`). c17 tracks a byte `offset` plus an open storage unit, and opens a new unit whenever `current_storage_unit_size != storage_size` — so two bitfields of different declared types never share a unit, giving `int a:4; unsigned b:4; signed char c:3;` eight bytes instead of four. After a plain member it resets the unit and re-aligns the *byte* offset to the bitfield's declared type, so `char x; unsigned a:1; char y;` pushes the unit to offset 4 and the struct to 12; gcc puts `a:1` at bit 8 and `y` at byte 2, for 4.

  The System V ABI does not work in units-and-bytes: it tracks a running **bit** offset from the start of the struct, and a bitfield's storage unit is a window that may begin anywhere, advancing to the next `sizeof(T)*8` boundary only when the field would otherwise cross one. Fixing this means reworking the bitfield half of `compute_struct_layout` to that model — every currently-passing row of `/tmp/bf.sh` (same-type packing, cross-unit, zero-width, `long long`, the narrow-type cases, `int x; unsigned a:1;`) must keep passing, and it changes layout for every struct, so it wants the CPython gate and a fresh differential matrix rather than a quick edit.

  Found by the Phase F sweep.

- [ ] **#C15 — A declaration with no declarator is accepted where gcc errors.** **Open, Minor.** **[probed]** `int int;`, `int register;`, `int inline;`, `int _Bool;`, `int __asm__;` and similar are accepted silently; gcc gives "two or more data types in declaration specifiers", "empty declaration with storage class specifier", or the equivalent. No symbol is emitted in any of these, so nothing is miscompiled — this is a missing diagnostic, not a wrong program, which is why it was separated from #C10 rather than folded into it. Found by the same probe.

- [ ] **#C16 — `alignof`, `offsetof` and `setjmp` cannot be *used* as ordinary identifiers.** **Open, Minor.** **[probed]** all three declare fine (`int offsetof;` is accepted) but fail in expression position: `offsetof = 1;` gives "expected '('" and `alignof = 1;` is a parse error. gcc accepts all three in C17 mode, since none is a C17 keyword. The expression parser special-cases the names ahead of ordinary identifier lookup. Distinct from #C10, which is about the declarator position only.
