...
/// (subject to growth limits — see LARGE_FUNCTION_GROWTH).
...
/// Maximum function size to inline at -O2 or higher (single call site)
const MAX_INLINE_SIZE: usize = 100;

/// Functions with at least this many instructions are considered "large."
/// Growth limits are enforced once a caller reaches this size.
/// GCC uses 2700; we use a lower value because pcc's stack frames grow
/// ~8 bytes per inlined instruction (no register promotion for addr-taken
/// locals), so frame bloat becomes significant earlier than in GCC.
const LARGE_FUNCTION_INSNS: usize = 500;

/// Maximum growth (%) allowed for large functions via inlining.
/// A function of size N can grow to N * (1 + LARGE_FUNCTION_GROWTH/100).
/// GCC default: 100 (2x). We use 50%.
const LARGE_FUNCTION_GROWTH: usize = 50;

/// Hard cap on caller size beyond which ALL inlining is disabled.
/// This is a pcc-specific workaround: without register promotion for
/// addr-taken locals, pcc's stack frames grow linearly with inlined code.
/// GCC/LLVM don't need this because they keep addr-taken locals in registers.
/// Functions this large (e.g., _PyEval_EvalFrameDefault at ~400K instructions)
/// gain little from inlining but suffer severe frame bloat.
/// TODO: remove this once register promotion is implemented.
const HARD_CALLER_SIZE_CAP: usize = 5000;

/// Maximum estimated stack frame (bytes) for recursive callers.
/// Beyond this, no inlining into the caller is allowed.
...
///
/// Uses a hybrid of GCC's growth model and LLVM's recursive-caller stack check:
/// - Callee size thresholds decide if inlining is *desirable*
/// - Proportional growth limit decides if the caller can *afford* it
...
fn should_inline(
    ...
    // Hard cap for extreme outliers (eval loops, autogenerated code).
    // pcc-specific: without register promotion, every inlined copy adds
    // ~8 bytes of stack. Functions with 5000+ instructions already have
    // large frames; inlining makes them worse. See HARD_CALLER_SIZE_CAP.
    if caller_size > HARD_CALLER_SIZE_CAP {
        return false;
    }

    // Recursive callers: allow inlining only if estimated stack is small.
    // LLVM uses 1024 bytes. We estimate stack as ~8 bytes per instruction
    // (conservative: most instructions don't need stack, but addr-taken
    // locals and spills add up in pcc's current register allocator).
    if caller_is_recursive {
        let estimated_stack = caller_size * 8;
    ...
    let desirable = if candidate.estimated_size <= ALWAYS_INLINE_SIZE
        || (candidate.has_inline_hint && candidate.estimated_size <= HINT_INLINE_SIZE)
    {
        ...
        if candidate.call_count == 1 && candidate.estimated_size <= MAX_INLINE_SIZE {
            ...
            candidate.estimated_size <= ALWAYS_INLINE_SIZE * 2
        }
    } else {
        false
    };
    ...
    // Growth limit (GCC-style): once a function is "large," limit how much
    // further it can grow. The limit is proportional to the current size.
    //
    ...
    if new_size > limit && new_size > LARGE_FUNCTION_INSNS {
        return false;
    }
...
fn clone_instruction(
    ...
    match insn.op {
        ...
        // which already holds the struct address (for MEMORY-class structs).
        // Convert symaddr to copy to avoid taking address-of-address.
        Opcode::SymAddr
            if insn.src.first().is_some_and(|&src| {
                matches!(
                    callee_func.get_pseudo(src).map(|p| &p.kind),
                    Some(PseudoKind::Arg(_))
                )
            }) =>
        {
            debug_assert_eq!(insn.src.len(), 1, "SymAddr should have exactly one source");
            let remapped_src = ctx.remap_pseudo(insn.src[0], callee_func);
            let mut copy_insn = Instruction::new(Opcode::Copy);
            copy_insn.target = insn.target.map(|t| ctx.remap_pseudo(t, callee_func));
            copy_insn.src = vec![remapped_src];
            copy_insn.typ = insn.typ;
            copy_insn.size = insn.size;
            copy_insn.pos = insn.pos;
            vec![copy_insn]
        }
