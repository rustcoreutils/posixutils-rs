...
- [ ] **#C17 — A bitfield narrower than `int` does not sign-extend on read.** **Open, Major.** **[probed]** `struct C { signed char c:3; }; c.c = -4;` reads back as `4` under c17 and `-4` under gcc — a silent wrong value in ordinary code. `int a:4` is correct, so the defect is specific to a bitfield whose declared type is narrower than `int`; the read appears to extend from the declared type's width rather than the field's. Found by the Phase F sweep.

- [ ] **#C18 — Struct layout is wrong when bitfields mix with other members.** **Open, Major.** **[probed]** `struct D { char x; unsigned a:1; char y; }` is 4 bytes under gcc and **12** under c17; `struct C { int a:4; unsigned b:4; signed char c:3; }` is 4 under gcc and 8 under c17. Both are ABI-visible: a struct passed between a c17 object and a gcc one is laid out differently, so this is worse than a size mismatch. The zero-width case (#C4) and the all-`unsigned`-same-type cases are correct, so the defect is in allocating a bitfield next to a non-bitfield member, and in mixing declared types. Found by the Phase F sweep.
