...
#[test]
fn test_exclusive_start_condition_backtracking() {
    // Test that YYMARKER is NOT updated for rules invalid in exclusive start conditions.
    // Bug: Previously, YYMARKER was set unconditionally at accepting states, causing
    // incorrect backtracking when the accepting rule was invalid for the current
    // start condition.
    //
    // Scenario:
    // 1. In exclusive state STRING, scan input that matches a pattern only valid in INITIAL
    // 2. The DFA should NOT set YYMARKER for that invalid rule
    // 3. Correct behavior: fall through to valid rules or default action
    let lex_input = r#"
%x STRING
%option noinput nounput
%%
\"              { printf("BEGIN_STRING\n"); BEGIN(STRING); }
<STRING>\"      { printf("END_STRING\n"); BEGIN(INITIAL); }
<STRING>[^\"\n]+  printf("STRING_CONTENT: %s\n", yytext);
<STRING>\n      { printf("UNTERMINATED_STRING\n"); BEGIN(INITIAL); }
hello           printf("KEYWORD_HELLO\n");
[a-z]+          printf("WORD: %s\n", yytext);
[ \t\n]+        /* skip */
.               printf("OTHER: %s\n", yytext);
%%
...
"#;
    ...
    // Test 1: Basic string parsing works
    let result = compile_and_run(&c_code, "\"test string\"\n").unwrap();
    assert!(result.contains("BEGIN_STRING"), "Should start string");
    assert!(
        result.contains("STRING_CONTENT: test string"),
        "Should capture content"
    );
    assert!(result.contains("END_STRING"), "Should end string");

    // Test 2: Keywords work outside strings
    let result = compile_and_run(&c_code, "hello world\n").unwrap();
    assert!(
        result.contains("KEYWORD_HELLO"),
        "Should match hello keyword"
    );
    assert!(result.contains("WORD: world"), "Should match word");

    // Test 3: Critical test - input inside string that would match INITIAL-only rules
    // The "hello" inside the string should NOT trigger KEYWORD_HELLO (invalid in STRING)
    // and should be captured as STRING_CONTENT
    let result = compile_and_run(&c_code, "\"hello world\"\n").unwrap();
    assert!(result.contains("BEGIN_STRING"), "Should start string");
    assert!(
        result.contains("STRING_CONTENT: hello world"),
        "Content with 'hello' should be captured as string content, not keyword"
    );
    assert!(
        !result.contains("KEYWORD_HELLO"),
