...
- [ ] **#C34 — aarch64 tells the caller's frame from the callee's by the sign of an offset.** **Open, Minor.** x86-64 has a `Loc::IncomingArg(i32)` distinct from `Loc::Stack(i32)`; aarch64 has only the latter, and a *positive* offset means an incoming stack argument while a *negative* one means a local or spill slot (`arch/aarch64/regalloc.rs`, `arch/aarch64/codegen.rs` `stack_offset`). Two address spaces share one integer type, so nothing stops one being assigned to the other, and `offset == 0` resolves to the incoming side. It has already caused one regression, fixed in place and documented at `arch/aarch64/regalloc.rs` — "prolog stores landed at `frame_size + offset - 16` (the incoming-stack-arg region) rather than the local frame, corrupting the caller's stack". Not a defect on its own: no current path assigns across the two. It is the aarch64 half of what #C33 fixed for x86-64, and the same hazard for any future pass that merges pseudos. Found while fixing #C33.
