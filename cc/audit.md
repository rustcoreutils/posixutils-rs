...
- [ ] **#C32 — Two aggregates aarch64 should treat as HFAs are not recognised.** **Open, Minor.** **[probed]** `is_hfa_base_type` admits `float`, `double` and a 64- or 128-bit `long double`, and the member walk recurses into nested structs but not into **array** members. So `struct { long double v[1]; }` is not classified an HFA and is returned in a general register where gcc uses V0, and `struct { _Float16 v; }` likewise, since `_Float16` is not among the accepted base types (AAPCS64 admits half precision). Both are self-consistent within a c17 translation unit and wrong against a gcc-compiled peer. Adding `_Float16` needs a new `HfaBase` variant. Found while fixing #C30.

- [ ] **#C25 — A `long double` argument following a `long double _Complex` one arrives swapped.** **Open, Major.** **[probed]** `probe(a, 1.5L, 2.5L)` with the signature `void probe(long double _Complex v, long double re, long double im)` prints `re=2.500 im=1.500` under c17 at `-O0` and `re=1.500 im=2.500` under gcc. The complex value itself is correct; the two scalars that follow it are exchanged. Not reproducible at `-O2`, which is why it reads as a stack-offset calculation for the arguments after a memory-class complex rather than a classification error. Found while writing the #C20 tests.

- [ ] **#C26 — Indexing an array member of a call-result rvalue segfaults.** **Open, Major.** **[probed]** `mka().v[0]` where `mka` returns `struct A { int v[2]; }` crashes: the caller loads the result local's *bytes* as an address and dereferences them. Not specific to any element type — `int`, `double` and `long double` all crash — and not related to the return class, since the same struct assigned to a local first reads correctly. C17 6.5.2.3 gives such a temporary lifetime until the end of the full expression, and gcc compiles it. Found while writing the #C21 tests.

- [ ] **#C27 — An integer or mixed struct of nine to sixteen bytes is passed as a pointer.** **Open, Major.** **[probed]** `struct { long a, b; }` and `struct { double a; int b; }` are passed as a pointer in a general register where System V passes them in two registers by value. Self-consistent within one c17 translation unit and wrong against every gcc-compiled peer: a c17 `take()` called from gcc-compiled code reads garbage. The all-SSE and MEMORY cases were fixed with #C21; this is the remaining third and is a wider change, since the two-register integer path does not exist yet on either side. Found while fixing #C21.

- [ ] **#C28 — A small all-SSE struct is passed in a general register.** **Open, Minor.** **[probed]** `struct { _Float16 v; }` and `struct { _Float16 a, b; }` are *returned* correctly since #C23, but as arguments c17 reads them from a general register while gcc passes them in `xmm0`. Eight bytes or fewer, so it is a different path from the nine-to-sixteen-byte one #C21 fixed. Found while fixing #C23.

- [ ] **#C29 — `__real__` and `__imag__` are not accepted.** **Open, Minor.** **[probed]** `__real__ z` gives "undeclared identifier '__real__'". Both are long-standing GCC extensions and the natural way to reach a complex value's halves without `<complex.h>`; c17 offers only `creal`/`cimag`, which need libm. Found while writing the #C20 tests.
