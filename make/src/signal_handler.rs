...
use crate::rule::{InterruptInfo, IN_FLIGHT};
...
/// uses `try_lock` on `IN_FLIGHT` so it can never deadlock against a build
/// thread that briefly holds that lock while recording the active target.
pub fn handle_signals(signal_code: libc::c_int) {
    ...
    let in_flight: Vec<InterruptInfo> = IN_FLIGHT
        .try_lock()
    ...
    // Under `-j` there is a recipe in flight per worker, and each one may have
    // left a partial file behind.
    for info in in_flight {
        delete_if_partial(&info);
    }
...
/// Delete a target whose recipe was interrupted part-way through writing it.
///
/// POSIX: delete the target only if it is not `.PRECIOUS`/`.PHONY` and its
/// modification time changed while the interrupted recipe ran, i.e. the recipe
/// had begun writing a partial file.
fn delete_if_partial(info: &InterruptInfo) {
    let current_mtime = fs::metadata(&info.target)
        .ok()
        .and_then(|m| m.modified().ok());
    if info.precious || info.phony || current_mtime == info.original_mtime {
        return;
    }
    eprintln!(
        "{}: {} '{}'",
