...
A template + collected wisdom for auditing any single posixutils-rs utility against POSIX.1-2024 (IEEE Std 1003.1-2024). Distilled from the first full audit (`more`, in the `display/` crate).
...
Every crate in the workspace has been audited. A crate keeps an `audit.md` only
while it still has an open item; when the last one closes the file goes away,
because a punch list with nothing on it is worse than no file — it reads as
...
trigraph reasoning is in `cc/TODO.md` under "Settled — do not re-open".
...
When you finish a new audit, add the link here — and when its last box closes,
delete the file and move the crate to the list above.
