...
- [x] **`-t` supported** (#2 ✓ Phase 3).
- [x] **`-P|-t` mutually exclusive** (#7 ✓ Phase 3).
- [x] `df` conforms to XBD 12.2 via clap default parsing.
- Note: `--kilo`/`--portable`/`--total` long aliases are non-POSIX extensions (harmless).
...
| `-t` | CONFORMS | (#2 ✓ Phase 3) `conflicts_with = "portable"`; shows inodes + total. |
...
- [x] **Default & `-t` output report free inodes** (#3 ✓ Phase 3) — `Inodes`/`IUsed`/`IFree`/`IUse%` columns from `f_files`/`f_ffree`.
- [x] **`-t` accepted; total-space always present** (#2 ✓ Phase 3).
...
- [x] **`<percentage used>` denominator** (#4 ✓ Phase 2) — `capacity_percent` now uses `used + avail` (`f_bavail`).
- [x] Percentage rounded up to next integer (`.ceil()`) — `capacity_percent`. CONFORMS.
- [x] **Unit scaling overflow-safe** (#10 ✓ Phase 2) — `scale_blocks` uses a `u128` intermediate.
