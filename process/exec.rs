...
//! (`env`, `nice`, …), which use `std::os::unix::process::CommandExt::exec`
//! to replace the process image.  `exec` only returns on failure, and POSIX
//! mandates distinct exit statuses for that failure:
//!
...
/// terminate the process. Emits a localized diagnostic via [`plib::diag`]
/// (so the caller must have run [`plib::diag::init_locale`] first), then:
///
