...
- [x] **#11 — `translit` with a single argument emits `0` and a mis-copied warning.** `builtin.rs:626-633` (the missing-second-arg arm, copied from `index`) writes `Warning too few arguments for index macro` and pushes back `0`. Verified: `translit(\`abc')` → `Warning too few arguments for index macro0`. Expected: the first argument unchanged (`abc`). Fix: with no second argument, push back the first argument verbatim. **✓ Fixed (Phase F):** the single-argument arm now returns the first argument unchanged. Fixture `translit_single_arg`.
- [x] **#12 — `changecom` with a single argument does not reset the end-comment string to `<newline>`.** `builtin.rs:316-342` sets `comment_close_tag` only when `args_len >= 2`; a prior two-arg `changecom` leaves a stale close tag in effect. POSIX (pp. 3092): "With a single non-null argument, that argument shall become the begin-comment and the `<newline>` shall become the end-comment string." Fix: in the one-argument branch, set the close tag to `\n`. **✓ Fixed (Phase F):** the one-argument branch resets `comment_close_tag` to `DEFAULT_COMMENT_CLOSE_TAG` (`\n`). Fixture `changecom_single_arg`.
- [x] **#13 — No `setlocale`/`gettext`: `LC_MESSAGES`, `LC_CTYPE`, `NLSPATH`, and `LANG` are ignored and all diagnostics are hardcoded English.** Confirmed: no `setlocale`/`gettext`/`textdomain` and no `LC_*`/`NLSPATH`/`LANG` reads anywhere in `m4/src` (`main.rs` only calls `env_logger::init`). POSIX ENVIRONMENT VARIABLES (pp. 3090) lists all four. Fix: add `setlocale(LC_ALL, "")` at startup and route diagnostics through `gettext` (and through `stderr` with an `m4:` prefix, cf. #9). **✓ Fixed (Phase E + i18n follow-up):** `main` now calls `plib::diag::init_locale("m4")` → `setlocale(LC_ALL, "")` + `textdomain("posixutils-rs")` + `bind_textdomain_codeset`, so `LC_ALL`/`LC_CTYPE`/`LANG` (and `NLSPATH`) take effect — this is what makes the #10 character handling locale-correct, and diagnostics carry the `m4:` prefix (Phase D). m4's user-facing strings (clap `-s`/`-D`/`-U` help and every recoverable diagnostic) are now wrapped in `gettext()`, matching the project convention (`text/nl.rs`, `text/tsort.rs`, …) and joining the shipped `posixutils-rs` catalog (`locale/{bn,es,hi,ja,pt,ru}/LC_MESSAGES/posixutils-rs.po`, compiled to `.mo` by the build); they translate once a locale's `msgstr`s are filled in (currently mostly empty, so English is shown — the same state as the other utilities).
...
- [x] **#16 — `-D`/`-U` are all applied before any file operand is processed, so they cannot interleave with operands.** `m4/src/lib.rs:179-195` applies the (correctly order-sorted) `-D`/`-U` directives in a block before `main_loop`. POSIX (pp. 3090): "options can be interspersed with operands." Verified the relative `-D`/`-U` order and basic interspersed parsing work (`m4 m4src -D VER=2` → defined), but `m4 -D X f1 -U X f2` would undefine `X` before `f1` is read. Fix: process `-D`/`-U` and file operands in a single command-line-ordered pass. (Lower-impact; most scripts define before the first operand.) **✓ Fixed (Phase F):** `Args` now keeps `-D`/`-U`/file operands in one command-line-ordered `Vec<InputItem>`; `run_impl` drives them in order, applying each `-D`/`-U` and processing each file (via the new `main_loop::process`) before the next item, with m4wrap/diversions flushed once at the true end (new `main_loop::finalize`). **This also fixed a latent bug the original audit missed (matrix wrongly marked "processed in order" CONFORMS — the `two_files` fixture is diversion-masked): multiple file operands were processed in *reverse* (input-stack LIFO). `m4 a b` now emits `a` then `b`.** Verified: `-D X=1 useX -U X useX` → `x=1` then `x=X`; a macro defined in file 1 is visible in file 2. Fixtures `define_undefine_operand_order`, `forward_file_order`.
...
- [x] **#13 `LC_MESSAGES` CONFORMS** (Phase E + i18n follow-up) — locale set, diagnostics `m4:`-prefixed and `gettext()`-wrapped; joins the shipped `posixutils-rs` catalog (translates when a locale's `msgstr`s are populated).
...
- [x] **#9/#17 CONFORMS** (Phases D + i18n follow-up) — recoverable diagnostics carry the GNU `m4:<file>:<line>: [Warning: ]` prefix via `State::emit_warning`/`emit_error`, and their message text is `gettext()`-wrapped (joins the `posixutils-rs` catalog — see #13).
