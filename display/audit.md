...
- [x] **#14 — `MoreError` strings hardcoded English.** Only clap help strings are gettext'd. `more.rs:198-246`. ✓ **fixed (Phase 8)**: `thiserror`'s derived `Display` evaluates its format strings at compile time and so cannot call `gettext`. All three error enums (`MoreError`, `SeekPositionsError`, `SourceContextError`) now carry a hand-written `Display` that passes every message through `gettext`, keeping the existing wording so the tests' expected diagnostics are unchanged. Tree-wide caveat: `.po` templates for six locales live under `locale/*/LC_MESSAGES/` and `make locale` compiles them with `msgfmt`, but every `msgstr` is still empty (883 msgids, none translated), so nothing is translated in practice yet.
...
- [x] **`NLSPATH` CONFORMS** (XSI, 93204) — `gettext-rs` consults `NLSPATH` ahead of `bindtextdomain`/`TEXTDOMAINDIR`/system dirs, with `%N`/`%L`/`%l`/`%t`/`%c` expansion (2026-08-04).
...
- [x] **`NLSPATH` CONFORMS** (XSI, 111927) — `gettext-rs` consults `NLSPATH` ahead of `bindtextdomain`/`TEXTDOMAINDIR`/system dirs, with `%N`/`%L`/`%l`/`%t`/`%c` expansion (2026-08-04).
...
|---|---|---|---|---|---|
| `more` | 2 | 7 | 6 | all 15 priority items and all matrix items fixed | Stage 6 — Audited |
...
4. **`NLSPATH`** is honored as of 2026-08-04 (`gettext-rs` reads GNU `.mo` catalogs and consults `NLSPATH` ahead of `bindtextdomain`/`TEXTDOMAINDIR`/system dirs). What remains is catalog *content*, not mechanism: `.po` templates for six locales live under `locale/*/LC_MESSAGES/` and `make locale` compiles them with `msgfmt`, but every `msgstr` is still empty (883 msgids, none translated), so nothing is translated in practice yet. Tracked at project level, not per-utility.

**Promotion status.** `echo` and `printf` are promoted to README **Stage 6 — Audited**: every actionable finding is fixed and covered by a regression test. The one item left unchecked for them is `echo` #E7, a deliberate, documented choice POSIX explicitly makes implementation-defined. (`NLSPATH` was the other; it is honored as of 2026-08-04.)

`more` keeps its Stage 6 standing, now with **every** numbered priority item and every matrix item closed. The four interactive-render items that were deferred for want of a PTY harness are fixed, the harness exists (`display/tests/common/mod.rs`), and ten further defects found during that work are recorded above — nine fixed, one (the unbounded line index) deliberately deferred with its measurement. `NLSPATH`, previously the only item left unchecked for the whole crate, is honored as of 2026-08-04; every item is now closed.
