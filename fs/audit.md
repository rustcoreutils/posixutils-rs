...
- [ ] **#1 — Non-UTF-8 mount device/path aborts the whole utility (panic).** `fs/df.rs:282-283` (`MountList::push`) does `devname.to_str().unwrap()` and `dirname.to_str().unwrap()`; `fs/df.rs:325` does `mount.dir.to_str().unwrap()` in the statfs error path. Mount `fsname`/`dir` come straight from the kernel mount table (`getmntent`) and may contain arbitrary, non-UTF-8 bytes (e.g. removable media auto-mounted under a label with high bytes). Any such entry makes the bare `df` invocation `panic!` and print nothing. Fix: carry device/mount names as `OsString`/`Vec<u8>` (or `to_string_lossy()`), never `unwrap()` on `to_str()`.
...
- [ ] **#8 — Newline in a displayed pathname is not treated as an error.** FUTURE DIRECTIONS (92502-92505) / Austin Group Defect 251 "encourages" implementations to error when an output pathname contains a `<newline>` (a separator in the `-P` format). Not yet mandatory; track only. Fix: when a `devname`/`dir` to be printed contains `\n`, diagnose and set non-zero exit.
- [ ] **#9 — File/device names and operands are forced through UTF-8 `String`.** `Args.files: Vec<String>` (`fs/df.rs:42`) and the `String` fields of `Mount` (`fs/df.rs:208-210`) mangle/refuse non-UTF-8 bytes. Subsumes #1's crash but is broader (operands too). Fix: use `OsString`/`PathBuf` end-to-end.
...
- [ ] **Unmatched/failed operand → prints all filesystems** (#5 Major) — `fs/df.rs:263-268`.
- [x] Non-special file operands accepted (results "unspecified" per 92394-92399 — legal). N/A.
- Note: operand→filesystem matching compares `file.st_dev` against a per-mount `dev` derived as `stat(devname).st_rdev` else `stat(dirname).st_dev` else `-1` (`fs/df.rs:270-288`); works for real block devices and via the dirname fallback for pseudo-filesystems, but is fragile and feeds #5.
...
| `LC_CTYPE` | CONFORMS (init) | `setlocale` honors it; byte-vs-UTF-8 handling still naive (#9). |
