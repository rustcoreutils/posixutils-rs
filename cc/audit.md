...
- [ ] **#H4 — aarch64 `long double` load/store collapses to 64-bit, contradicting its own 128-bit ABI.** **CI has now adjudicated it: half confirmed, half misdiagnosed.** `c17_long_double_round_trip` failed on macOS/aarch64 — but not for the reason the finding gives. Apple makes `long double` a 64-bit `double`, so mapping it to `FpSize::Double` there is *correct*; what was wrong is that the `__LDBL_*` predefines were hardcoded to x86_64's 80-bit values on every target, so `<float.h>` reported `LDBL_EPSILON` as 1.08e-19 for a type with a 53-bit mantissa and `1.0L + LDBL_EPSILON` compared equal to `1.0L`. That is fixed, along with `__SIZEOF_LONG_DOUBLE__`, and the duplicate long-double macros in the two per-arch lists (which could not see the OS) are gone. **What remains open is aarch64 *Linux*, which no CI job covers**: there `long double` is IEEE binary128, `cc/types.rs` sizes it at 128 bits, and `cc/arch/aarch64/float.rs:29` still loads and stores 64 — the type table and the codegen disagree. It is worse than a truncation for the complex form: `long double _Complex` on that target reaches `unreachable!("x87 extended not available on AArch64")` in `cc/arch/aarch64/lir.rs:1440` and the compiler panics. **[probed via `--target aarch64-unknown-linux-gnu`; pre-existing, not a regression.]** Closing it means either implementing binary128 (soft-float via libgcc's `__addtf3`/`__multf3` family, which `aarch64/mapping.rs` already names) or deciding to make `long double` 64-bit there and documenting the ABI divergence. Both are real work and neither should be chosen without an aarch64 Linux runner. Original finding, still accurate for that target: `c17_long_double_round_trip` round-trips a value needing more than 53 bits of mantissa through a static, a function return and an array — the shapes that would expose a 64-bit slot. It passes on x86_64; aarch64 CI adjudicates the claim. Deliberately **not** "fixed" on the static reading, per the finding's own advice. Original finding: `cc/arch/aarch64/float.rs:24-33` maps `TypeKind::Double | TypeKind::LongDouble => FpSize::Double`, while the generic default at `cc/arch/lir.rs:114-115` uses `FpSize::Extended`. Everything around it is consistent with 128-bit: `cc/arch/aarch64/macros.rs:32` advertises `__LDBL_MANT_DIG__ = 113`, `cc/types.rs:1096-1101` sizes `long double` at 128 bits on aarch64/Linux, and `cc/arch/aarch64/mapping.rs:38-215` routes long-double arithmetic to libgcc's IEEE-quad `__addtf3`/`__multf3`/… family. If the load/store path feeding those calls truncates to 64 bits, `long double` is corrupted round-trip on aarch64/Linux. **[static — NOT VERIFIED; this audit ran on x86_64 and the claim is aarch64-only.]** Treat as a lead, not a confirmed defect: write a `long double` round-trip test and run it on aarch64 before scheduling work.
...
- [ ] **#H13 — AArch64 does not pass a complex argument that runs out of FP registers.** `cc/arch/aarch64/codegen.rs:1021` copies a complex parameter from its two V registers into the local only when `fp_arg_idx + 1 < fp_arg_regs.len()`; when it does not fit, nothing is stored and the parameter is uninitialized. The same defect on x86_64 is fixed here, but the fix does not transfer: System V leaves the registers an over-large argument did not fit into available to later arguments, whereas AAPCS64 §6.4.2 sets NSRN to 8 as soon as anything is laid out on the stack, so every subsequent floating-point argument goes there too. `c99_complex_argument_spilled_to_the_stack` covers the x86_64 side and is `#[cfg(target_arch = "x86_64")]` for exactly this reason. **[static — found while fixing the x86_64 path; needs an aarch64 machine to develop and verify against, which no CI job currently provides.]**

- [ ] **#C5 — A cast to a narrower type is dropped when the value is passed to a variadic function.** **[probed]** `printf("%02x", (unsigned char)c)` with `signed char c = -128;` prints `ffffff80`; gcc prints `80`. Assigning through a variable first (`unsigned char u = (unsigned char)c; printf(..., u)`) is correct, as is an outer `(unsigned)` cast, so the value and the conversion both work — what fails is the default argument promotion, which widens from the operand's original type rather than the cast's. Found incidentally while fixing the string-literal byte handling; unrelated to it and to anything in the review.
