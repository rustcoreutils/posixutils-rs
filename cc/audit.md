...
- [ ] **#C41 — aarch64 passes a composite of at most sixteen bytes by pointer, not in registers.** **Open, Major — pre-existing, breaks every c17/gcc boundary.** AAPCS64 §5.4.2 stage C.10 puts a composite of sixteen bytes or fewer in consecutive X registers. c17's aarch64 caller hands over its *address* instead, and its callee reads one, so c17/c17 agrees with itself and nothing in-tree noticed. Against gcc it is wrong in both directions: `struct { long a, b; }` by value gives garbage with a c17 caller and **segfaults** with a gcc caller. `struct { int a; long b; }` likewise. The linearizer already records why — its `reg_pair` test is gated `arch == X86_64`, with the comment "aarch64 still hands such a struct over as a pointer". Surfaced by #C39: `va_arg` now follows the ABI, so it disagrees with c17's own caller. A sixteen-byte integer composite is held out of `codegen_variadic_aggregate_results_coexist` until this is fixed.
