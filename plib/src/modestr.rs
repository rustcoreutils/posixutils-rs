...
pub fn mutate(init_mode: u32, is_dir: bool, symbolic: &ChmodSymbolic) -> u32 {
    ...
    let mut get_umask = || -> u32 {
        match cached_umask {
            Some(m) => m,
            None => {
                // NOTE: Potential race condition on umask here
                let m = unsafe {
                    let m = libc::umask(0);

                    // We're only querying the umask so must change it back
                    libc::umask(m);

                    // Cast to `u32` because `mode_t` in macOS is `u16`
                    m as u32
                };
    ...
    for clause in &symbolic.clauses {
        ...
        for action in &clause.actions {
            ...
            let has_any_exec_bits = ((user | group | others) & (S_IXUSR | S_IXGRP | S_IXOTH)) != 0;
