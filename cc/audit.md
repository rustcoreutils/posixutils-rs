...
- [ ] **#C11 — A `_Complex` object with static storage duration cannot be initialized.** **Open, Minor.** **[probed]** at file scope both `double _Complex z = 1.0 + 2.0*I;` and `double _Complex z = CMPLX(1.0, 2.0);` are rejected with "unsupported expression in global initializer"; gcc accepts both. C99 6.7.8 allows any constant expression there, and `I` is `__builtin_complex(0.0, 1.0)`, so the builtin simply has no constant-folding path into `Initializer`. The same expressions work inside a function. Two smaller faults ride along: the diagnostic prints a raw Rust `Debug` dump of the AST — internal representation leaking into a user-facing message — and reports the position as `:0` rather than the initializer's.

- [ ] **#C12 — `_FORTIFY_SOURCE` compiles but fortifies nothing.** **Open, Minor.** `__builtin_object_size` always answers `(size_t)-1` ("unknown") where gcc computes the real size — **[probed]** `char b[10]` gives 10 under gcc and `18446744073709551615` under c17. glibc's fortified headers are written to fall back to the unchecked call when the size is unknown, so `-D_FORTIFY_SOURCE=2` builds correctly and silently buys no checking at all. The `__builtin___*_chk` family is present and compiles, so nothing fails loudly. Not a conformance issue — the fortification is a glibc extension — but a security-relevant one for anyone who sets the flag expecting it to work.
