...
impl X86_64CodeGen {
    ...
    fn emit_inline_asm(&mut self, insn: &Instruction) {
        ...
        for input in &asm_data.inputs {
            let loc = self.get_location(input.pseudo);
            match loc {
                Loc::Reg(r) => {
                    operand_regs.push(Some(r));
                    operand_mem.push(None);
        ...
        // Substitute %0, %1, etc. in the template with actual operands
        let asm_output =
            self.substitute_asm_operands(&asm_data.template, &operand_regs, &operand_mem);
        ...
        // No need to move outputs - the asm wrote directly to the assigned locations
        // The subsequent Store instructions generated by the linearizer will handle
        // moving from the output pseudo to the final destination variable.
    }
    ...
    /// Substitute %0, %1, etc. with actual operand strings
                    ...
                    Some(&d) if d == 'b' || d == 'w' || d == 'k' || d == 'q' => {
                        ...
                        if let Some(&digit) = chars.peek() {
                            if digit.is_ascii_digit() {
                                chars.next();
                                let idx = (digit as usize) - ('0' as usize);
                                if idx < regs.len() {
                                    if let Some(ref mem) = mems[idx] {
                                        result.push_str(mem);
                                    } else if let Some(reg) = regs[idx] {
