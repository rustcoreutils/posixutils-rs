...
/// Returns: (table, check, pact, pgoto, defact, defgoto, consistent)
/// - consistent: for each state, true if only one reduce action exists (no shifts)
...
fn generate_action_goto_tables<W: Write>(
    ...
    // Build packed tables
    let (table, check, pact, pgoto, defact, defgoto, consistent) =
    ...
    writeln!(w, "{{")?;
    for chunk in consistent.chunks(10) {
        write!(w, "    ")?;
        for (i, &val) in chunk.iter().enumerate() {
            if i > 0 {
                write!(w, ",")?;
...
fn build_packed_tables(
    ...
    (table, check, pact, pgoto, defact, defgoto, consistent)
}
