...
- [ ] **#C38 — An inlined function with a stacked HFA parameter stores through an undefined register.** **Open, Major — aarch64 only, long-standing.** `double f(F4 p, F4 q, D2 r)` called with all three by value: `p` and `q` fill V0-V7, `r` goes on the stack. When `f` is inlined, the implicit parameter copies the inliner generates for `p` and `q` — the ones standing in for what the backend prologue would have done — store to a destination that regalloc placed in `x3`, which nothing defines. It segfaults at `-O0` as well as `-O`. Not a regression: it reproduces on `0e58f972`, before any of this session's work. x86-64 compiles the same source correctly, and the same shape with `noinline`, or with every HFA parameter in registers, is also correct — so it is the combination of inlining with a parameter list that overflows the V registers. Reproducer in `codegen_stacked_hfa_element_counts`'s comment; that test uses `noinline` precisely so this defect does not mask the one it covers. Found while fixing #C38a.
...
- [ ] **#C42 — An `__int128` constant argument crashes the x86-64 backend.** **Open, Major — pre-existing, a compiler panic.** `f((__int128)424242)` aborts with `int128_hi_mem_loc: expected stack loc, got Imm(424242)`. The lo/hi helpers (`arch/x86_64/codegen.rs`) return a `MemAddr` and so require the value to be in memory, but an `__int128` *constant* is left in `Loc::Imm`. The fix belongs in the allocator — an `__int128` constant should get a stack slot, as one arriving in a register pair already does — rather than at each of the ten call sites. Found while testing #C42a; that test passes its values through variables to avoid it.

- [ ] **#C43 — x86-64 does not align a stacked sixteen-byte-aligned argument.** **Open, Major — pre-existing.** SysV AMD64 §3.2.3 requires a stacked argument whose type has sixteen-byte alignment to sit at a sixteen-byte boundary. This backend *pushes* stacked arguments rather than placing them at computed offsets (`push_stack_args`, `arch/x86_64/call.rs`), so where a sixteen-byte-aligned one lands depends on the running push count: a stacked `__int128` after an odd number of eight-byte arguments is eight bytes low, exactly as aarch64's was in #C42a. Fixing it means placing arguments at computed offsets rather than pushing, which is why it is not folded into #C42a. `codegen_stacked_argument_alignment` is gated to aarch64 until it is done.
...
- [ ] **#C52 — `sizeof` of a variably-modified type-name answers 0.** **Open, Minor — pre-existing.** `sizeof(int[n])` is 0 where gcc evaluates it at run time. Measured before the type-name rework, so not a regression from it: the old type-name parser wrote a zero size for a non-constant dimension and `parse_declarator` writes an absent one, and neither carries the size expression to `sizeof`. Fixing it means giving a variably-modified type-name its size expression and evaluating it — a feature, not a repair.

- [ ] **#C53 — A trailing comma is accepted in a parameter list.** **Open, Trivial — pre-existing.** `void g(int, );` compiles; C17 requires a declaration specifier or `...` after the comma. C23 allows it. Harmless, and the kind of thing the constraint work could pick up.

- [ ] **#C54 — Read-only accessors on the type table are `#[cfg(test)]`-gated.** **Open, Minor — a source of duplication, not a defect.** `format_type`, `composite`, `array_size` were all test-only, so production code that needed them hand-rolled the same lookups inline; the 2026-08-15 series had to un-gate all three to name types in diagnostics. `cflow.rs:643` still carries its own fourth `format_type`. Relatedly, `parse_function_def` (`parse/parser.rs`) is `#[cfg(test)]`-only, so `cargo build` and `cargo test` compile different parser entry points — a stale call site there is invisible to a release build and only `clippy --all-targets` catches it. The same duplicate-implementation hazard #C44a removed from declarator parsing.
