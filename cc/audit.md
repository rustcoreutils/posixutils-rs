...
- [x] **#P1/#X2 — `__STDC_VERSION__` is `201112L`; the compiler cannot claim C17.** **✓ fixed.** A `CStd`/`LangOpts` pair in `cc/target.rs` threads the dialect through `Preprocessor::new` and `PreprocessConfig`; the default is `gnu17`, which POSIX.2024's `c17` requires (87830 binds the language to ISO/IEC 9899:2018). `-std=` is parsed rather than discarded, and an unknown value is an error. The dialect now drives `__STDC_VERSION__`, `__STRICT_ANSI__`, the C11-only `__STDC_UTF_16/32__` and `__STDC_NO_THREADS__`, and whether non-reserved predefines (`unix`, `linux`) exist — matching gcc's table exactly. The test-only copy of the argument rewriter, which had its own `-std=` arm and would have drifted again, is folded into the real one.
