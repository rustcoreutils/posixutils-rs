...
- [x] **#1 — `-m` (mail after printing) is a parsed-but-ignored no-op.** ✓ **fixed (Phase 4).** The field is now `mail` and consulted: after all jobs are submitted, `notify_after_print` polls each job to a terminal state (`wait_for_job`, bounded by `POLL_TIMEOUT` = 120 s) and then `send_mail` delivers a completion summary to the requesting user via the local `sendmail` (`-t -oi`), modeled on `cron/job.rs::mail_output`. Recipient is gated through `recipient_is_safe` (rejects control/whitespace/`<`/`>`). Best-effort. Unit-tested: `recipient_is_safe_*`. (The end-to-end poll/mail path needs a live IPP server, unverifiable in the harness.)
- [x] **#2 — `-w` (write to terminal after printing) is a parsed-but-ignored no-op.** ✓ **fixed (Phase 4).** The field is now `write_terminal`; the same post-submission poll then writes the completion summary to `/dev/tty` via the non-panicking `write_to_terminal` helper (modeled on `users/write.rs`). Best-effort; same harness verification limit as #1.
