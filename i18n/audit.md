...
4. **[RESOLVED]** **`LC_CTYPE`-driven byte interpretation ignored.** ✓ fixed 2026-08-08. The parsers read input as UTF-8 (`read_to_string`), so a message file, `.po` or locale source in any other codeset — Latin-1, EUC-JP, Shift-JIS — was **rejected outright**, before a single line had been looked at, with an opaque "stream did not contain valid UTF-8". The original note also listed `iconv`, which was wrong: `iconv` has always been byte-oriented (`iconv.rs:84` reads into a `[u8]` buffer), as the whole point of that utility. **`gencat`** is now byte-oriented end to end (`parse_bytes` replaces `parse_str`, `Msg.msg` is `Vec<u8>`), and where the grammar means a *character* rather than a byte — `$quote c`, and scanning for the `\` that introduces an escape — it uses `plib::locale::mb_char_slices`, which walks the bytes with `mbrtowc` under the current `LC_CTYPE`, so a multibyte character whose trailing byte happens to be `0x5C` can no longer be mistaken for a backslash; reading an *existing* catalog to merge into keeps its bytes too. **`msgfmt`** likewise: `PoEntry`'s `msgid`, `msgid_plural`, `msgstr`, `msgctxt` and `previous_msgid` are `Vec<u8>`, the `.po` reader works on byte lines and the `.mo` writer takes bytes — which also fixed a latent corruption unrelated to locales, since `\xNN` and `\ooo` escapes were materialised with `char::from(value as u8)` and so **every escape above 0x7F was UTF-8 encoded into two bytes** on the way into the `.mo`; comments, flags and domain names stay text, as they never reach the `.mo`. **`localedef`** reads its sources, `include`d files and charmap as bytes; its grammar is ASCII so those bytes are decoded for parsing, and non-ASCII character *data* is today only validated, never emitted, because compiling a real locale is the deferred LD-1 — when that lands the data will need to be carried as bytes rather than decoded. Covered by tests that feed a lone `0xE9` through each utility and assert it arrives as one byte, not as the two-byte UTF-8 encoding of U+00E9.
...
- [x] **`LC_CTYPE`** ✓ fixed (theme 4, 2026-08-08) — the source is read as bytes and characters are scanned under the current `LC_CTYPE` via `mb_char_slices`, so a non-UTF-8 message file is accepted and its bytes reach the catalog verbatim.
...
- [x] **`LC_CTYPE`** ✓ fixed (theme 4, 2026-08-08) — `.po` input is read as bytes and the message strings reach the `.mo` verbatim; this also fixed `\xNN`/`\ooo` escapes above 0x7F, which had been UTF-8 encoded into two bytes. `NLSPATH` CONFORMS — `gettext-rs` consults `NLSPATH` ahead of `bindtextdomain`/`TEXTDOMAINDIR`/system dirs, with `%N`/`%L`/`%l`/`%t`/`%c` expansion (2026-08-04). `LANGUAGE` N/A for the compiler.
