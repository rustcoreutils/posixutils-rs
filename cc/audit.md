...
| Include cycle/depth guard | CONFORMS **[probed 2026-08-21]** | a cycle exits 1, no hang, bounded by `max_include_depth = 200` as gcc's is. **Changed 2026-08-21:** it used to be an immediate "recursive include" error keyed on a set of files currently open. That error was unreachable for a *guarded* self-include only because the include-guard fast path skipped the file first -- by guessing, which is what deleted source in three other shapes. With the guessing gone, an immediate cycle error would have rejected the legal counter-guard idiom `#ifndef DEPTH / #define DEPTH 1 / #include self / #else ... #endif`, which gcc accepts |
| Macro expansion (6.10.3) | CONFORMS **[probed 2026-08-21]** | blue paint, rescanning including C17 6.10.3.4 EXAMPLE 3, mutual recursion, `##` placemarkers and validity (6.10.3.3p3), empty arguments, arity diagnostics, distinct-parameter diagnostics (6.10.3p6), `__VA_ARGS__`, and C23 6.10.5 `__VA_OPT__` including `#`, `##` and nested parentheses. **Three claims here were overstated until 2026-08-21:** rescanning ran in an isolated pass that could not see the rest of the file, so EXAMPLE 3 was rejected outright; `##` kept whatever re-lexing produced, so `cat(/,/)` made a comment and silently deleted the tokens on either side; and a duplicate macro parameter was accepted, making `#define F(a,a) a` expand `F(1,2)` to `1`. `#__VA_ARGS__` kept only its first argument until #C156. One divergence remains, pinned by `preprocessor_va_args_loses_space_before_a_separator`: the splitter discards the separating comma, so `V(a , b)` stringifies as `"a, b"` where gcc gives `"a , b"` |
| `#if` constant expressions | CONFORMS **[probed 2026-08-21]** | `intmax_t` arithmetic, arithmetic right shift, signed wraparound, unqualified-`u` promotion, short-circuit around `1/0`, character constants, `0xFFFFFFFFFFFFFFFF`. **The character-constant claim was overstated until 2026-08-21:** the evaluator packed the source spelling between the quotes, escapes undecoded, so `'\n'` was 23662 and `#if '\0'` was *true*. It now shares the parser's decoder (`cc/token/literal.rs`) and applies `char_signed`. The row was also silent about there being no error path at all -- `#if (1`, `#if X = 2`, `#if 1.5` each picked a branch without a diagnostic |
...
of strictly-conforming source: C23 `__VA_OPT__` (which gcc and clang accept in
every mode, and which the GNU `, ## __VA_ARGS__` extension now lowers onto),
`__attribute__`, statement expressions,
