...
#### Critical
- [ ] **#F1 — Leaf-component symlink-swap TOCTOU on directory descent (race → arbitrary-directory deletion via `rm`).** `ftw/src/lib.rs:918` → `ftw/src/dir.rs:94-95` (`OwnedDir::open_at` → `openat(dir_fd, name, O_RDONLY)`). `process_file` captures the entry type with a **non-following** `fstatat` at `lib.rs:506-510`, the consumer's `file_handler` then runs (for `rm -i` this *blocks on a prompt*, widening the window unboundedly), and only afterward does the engine open the child for descent. The descent open carries **no `O_NOFOLLOW` and no `O_DIRECTORY`** (`grep -nE 'O_NOFOLLOW|O_DIRECTORY' ftw/src/*.rs` → 0 matches), and the resulting fd is **never `fstat`'d and compared** against the metadata captured pre-descent (the `entry.metadata` stored into `TreeNode` at `lib.rs:900/912/922` is the original NOFOLLOW stat, never re-checked). An attacker who can replace a subdirectory entry with a symlink in the window between the stat and the open redirects the walk into an arbitrary directory; for `rm -r` the subsequent `unlinkat(new_dir_fd, …)` calls then delete the *attacker-chosen* directory's contents. This is the historically-exploited `rm -rf` race that coreutils `fts`/`remove.c` defends against precisely by opening with `O_NOFOLLOW|O_DIRECTORY|O_NONBLOCK` and verifying `dev`/`ino` on the opened fd. Exploitability requires winning a tight kernel race inside a directory the victim traverses and the attacker can write to (e.g. root running `rm -rf` over an attacker-owned tree, or any walk through a world-writable directory) — realistic in shared-filesystem / multi-user threat models. Fix: open descents with `O_RDONLY|O_NOFOLLOW|O_DIRECTORY|O_NONBLOCK`, `fstat` the fd, and abort the descent on `dev`/`ino` mismatch with the pre-descent stat (or on `ELOOP`/`ENOTDIR`).

#### Minor
- [ ] **#F2 — `panic!` on `getrlimit` failure aborts the process.** `ftw/src/lib.rs:793` (`panic!("{}", io::Error::last_os_error())`). A failed `getrlimit(RLIMIT_NOFILE)` kills the walk instead of falling back to a conservative fd budget. Fix: default the threshold and continue.
- [ ] **#F3 — `DeferredDir` fd re-open uses `.unwrap()` — a mid-traversal race panics.** `ftw/src/dir.rs:167,184,188,190,192` (`OwnedDir::new(...).unwrap()`, `open_long_filename(...).unwrap()`, `FileDescriptor::open_at(...).unwrap()`). On deep trees (subdirs beyond `RLIMIT_NOFILE − 7`) directories are reopened on demand from the retained parent fd; if that reopen fails (permissions changed, entry removed, or an adversarial race) the process panics instead of routing through `err_reporter`. Fix: propagate the error to `err_reporter` and mark the walk unsuccessful.
- [ ] **#F4 — `unreachable!()` on unknown `S_IFMT` aborts the process.** `ftw/src/lib.rs:162` (`Metadata::file_type`). A file whose mode bits are not one of the seven recognized types panics the whole utility. Fix: return a dedicated `Unknown` variant and let consumers skip/diagnose.
- [ ] **#F5 — Symlink-cycle detection lives only on the `DeferredDir` slow path.** `ftw/src/dir.rs:153,236-241` keeps a `visited: HashSet<ino_t>` only for deferred directories; the `OwnedDir` fast path relies solely on the unconditional `.`/`..` prune (`lib.rs:566`) plus the symlink opt-in. With `follow_symlinks` enabled (`cp -L`/`-H`), a symlink cycle that stays within the fd budget is not caught by an inode set. Fix: track a `(dev, ino)` set across the active stack when following symlinks.
...
#### Critical
- [ ] **#C1 — `cp -p` does not clear `S_ISUID`/`S_ISGID` when ownership can't be duplicated (privilege leak).** `tree/common/copy.rs:793-803` ignores the `fchownat` result (`errno::set_errno(0)` on failure), then `:806-811` applies the **full source mode** (`source_md.mode()`) via `fchmodat`, re-setting the setuid/setgid bits on a file now owned by the copying user. POSIX 90720-90721: *"If the user ID or the group ID cannot be duplicated, the file permission bits S_ISUID and S_ISGID shall be cleared."* (RATIONALE 90840-90842 stresses this is a deliberate anti-privilege-escalation measure.) Behaviorally: as non-root, `cp -p /usr/bin/passwd ./x` → `chown` to root fails, is swallowed, and `./x` ends up `-rwsr-xr-x caller:caller` (`4755`); GNU `cp -p` clears it to `0755`. Plain `cp` (no `-p`) is **fine** — it creates with the source mode and the kernel drops setuid, yielding `0755` (verified, matches GNU). Fix: capture whether `fchownat` succeeded (and whether the resulting owner/group equals the source's); if not, mask `~(S_ISUID|S_ISGID)` out of the mode before `fchmodat`.

#### Major
- [ ] **#C2 — >2 operands with a non-directory target silently copies only the first source.** `tree/cp.rs:139-156`: when `fs::metadata(target)` says the target is not a directory, `main` takes the single-file branch and calls `copy_file(&sources[0], target, …)`, **dropping `sources[1..]`** and exiting 0. POSIX 90605-90606: *"It shall be an error if target does not exist and more than two operands are specified, or if target exists and does not name a directory."* Behaviorally: `cp a b c` (c not a dir) → `c` gets `a`'s contents, `b` is ignored, **exit 0**; GNU prints `cp: target 'c': Not a directory` and exits 1. Fix: when `sources.len() > 1` and the target is not an existing directory, diagnose and exit non-zero before copying anything.
...
#### Critical
- [ ] **#M1 — Cross-filesystem `mv` does not clear `S_ISUID`/`S_ISGID` when ownership can't be duplicated.** `tree/mv.rs:80` sets `preserve = true` and routes through the shared `copy_characteristics` (`copy.rs:793-811`), which has the #C1 defect. POSIX 108104-108105: *"If the user ID, group ID, or file mode of a regular file cannot be duplicated, the file mode bits S_ISUID and S_ISGID shall not be duplicated."* Same root cause and fix as #C1 (shared engine).

#### Major
- [ ] **#M2 — >2 operands with a non-directory target silently moves only the first source.** `tree/mv.rs:413-433`: when `fs::metadata(target)` says the final operand is not a directory, `main` takes the single-move branch on `&sources[0]` and **drops `sources[1..]`**. Behaviorally: `mv a b c` (c not a dir) → `a` is renamed to `c` (**`a` is gone**), `b` is ignored, **exit 0**; GNU prints `mv: target 'c' is not a directory` and exits 1. More damaging than #C2 because the first source is actually removed. Fix: reject `sources.len() > 1` with a non-directory target before moving anything.
