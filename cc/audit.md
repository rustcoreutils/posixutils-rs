...
- [x] **#C85 — `__mode__` was unimplemented, and gave `register_t` the wrong size.** **✓ fixed (2026-08-18).** **[probed]** Filed as noise -- 567 warnings in a CPython build -- and the noise was the smaller half. glibc declares `register_t` with `__attribute__((__mode__(__word__)))`, so c17 sized it **4 bytes against gcc's 8**: a wrong type, silently, in a header any program may include. A machine mode names a width, and the attribute replaces the declared type with the one of that width in the same family, keeping the *declared* signedness -- `typedef unsigned u8 __attribute__((mode(QI)));` is unsigned and the `int` spelling signed. Integer modes `QI`/`HI`/`SI`/`DI`/`TI` and the synonyms `word`/`pointer`; floating `HF`/`SF`/`DF`/`XF`/`TF`; and the complex forms `HC`/`SC`/`DC`/`XC`/`TC`, which were 285 of the warnings on their own -- `<bits/floatn.h>` declares `__cfloat128` with `mode(TC)`. `XF` and `TF` are both sixteen bytes and **not** interchangeable, one being the x87 extended format and the other IEEE binary128, so they map to their own types rather than to a width; the test divides by three and checks the precision rather than trusting `sizeof`. A vector mode still warns, since ignoring one would change what the program computes and vector types do not exist here. A full CPython -O2 build now emits **zero** mode warnings, down from 567, and 552 diagnostics in total against 1,105. Tests: `c99_mode_attribute_selects_the_type` (widths, signedness from the declared type, complex halves, binary128 precision and narrow-integer wrapping), `diagnostics_implemented_modes_are_silent`, and the vector-mode warning in `diagnostics_unimplemented_attributes_are_reported`.
...
- [x] **#C102 — Reading an object in a static initializer was neither folded nor diagnosed.** **✓ fixed (2026-08-18).** **[probed]** One missing mechanism behind three symptoms, and the middle one is why a first attempt was reverted during the #C101 work. `int v; int w = v;` was accepted and silently yielded **zero**; `const int c = 5; int w = c;` was accepted and right, but only because nothing folded it; and `int w = c + 1;` one step along was **rejected**, valid code gcc compiles. Fixing any one alone leaves the others wrong. gcc's rule, measured: a `const`-qualified object *with a visible constant initializer* folds in a static initializer, in arbitrary arithmetic and at every arithmetic type, silently and even under `-pedantic`; without an initializer, or through `extern`, it does not; a non-`const` object never does. The folding is scoped by a **`ConstScope` parameter** threaded through the evaluator rather than a flag on the linearizer, because it is a property of the question being asked and not of compiler state -- a flag would let the answer depend on evaluation order. C makes a `const` object no kind of constant expression, so the lax scope must not reach an array size, a `case` label, `_Static_assert`, an enumerator or a bit-field width, all of which stay strict here as in gcc and all of which are pinned. `eval_static_address` moved to the lax scope too, being reachable only from the initializer path, which is what makes `int *p = &a[c-3];` compile. Tests: `diagnostics_non_constant_static_initializers_are_rejected` (seven), `diagnostics_const_objects_fold_in_static_initializers` (thirteen), `diagnostics_const_objects_do_not_fold_outside_initializers` (five -- the boundary), and `codegen_const_objects_fold_in_static_initializers` for the values, since acceptance alone would have passed before the fix.
...
- [ ] **#C114 — A floating constant expression is refused by `_Static_assert`.** **Open, Minor.** **[probed]** `_Static_assert(1.5 > 1.0, "x");` is rejected here and accepted by gcc. 6.6p4 makes an *integer* constant expression the requirement for an array size, a `case` label and an enumerator -- and c17 is right to refuse a floating one there -- but 6.7.10p1 asks `_Static_assert` for an integer constant expression too, and `1.5 > 1.0` **is** one: the comparison's result is `int`. The operands being floating does not make the expression floating. So the constraint is real and c17 applies it one level too early, rejecting on the operand type rather than the result type. Found while removing the now-dead strict entry point to the floating evaluator during #C102, which is how it became visible: no strict context reaches that evaluator at all, and this is the one that should.
...
- [x] **#C62 — `__builtin_*_overflow` with mixed signedness and a 128-bit destination.** **✓ fixed (2026-08-18).** **[probed]** `__builtin_add_overflow(-1, 5u, &u128)` reported overflow where the mathematical answer is 4; so did `(-10, 20)`. The builtin asks whether the *mathematical* result is representable, and the ordinary lowering answers that by computing in a type twice the destination's width and asking whether narrowing lost anything -- which has nothing to compare against once the destination is itself 128 bits, so a fallback examined the result directly on operands **already converted to the destination**. That conversion is not value-preserving for a negative operand with an unsigned destination, so the answer followed the converted value. The entry recorded this as needing 129 bits, and for two genuinely 128-bit operands it still does; but when both operands are *narrower* the exact computation fits and only the **check** has to change, from "did narrowing lose anything" to "is the exact value representable" -- which for a signed exact value in an unsigned destination is just "is it negative". The bound is counted in magnitude bits rather than assumed, and that is the part worth keeping: `(u64)-1 * (u64)-1` is just under 2^128, which `unsigned __int128` holds and `__int128` does not, so a shortcut gated on "narrower than the destination" alone would have let the exact computation wrap and then checked a value it never saw. A pre-existing test caught exactly that. Verified against gcc on 24 rows including both 128-bit destinations, full-width unsigned operands, `MIN * -1`, and the narrow destinations, which are untouched; and on aarch64 under qemu. Test: `builtins_checked_arith_128bit_mixed_signedness`.
