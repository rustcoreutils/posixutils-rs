...
## Status: CPython 3.12.9 builds and runs with pcc at -O0. Six bugs fixed this session (W-Z, char signedness, static local struct). Finalization crash FIXED. Pickle FIXED. Many tests pass. Remaining: intermittent ASLR-dependent segfaults, some test failures.
...
### Remaining: Intermittent ASLR-dependent segfaults
- Some tests crash intermittently with ASLR enabled but pass with `setarch --addr-no-randomize`
- Likely related to `_PyEval_EvalFrameDefault` 118KB stack frame + valgrind "below stack pointer" errors
- pcc allocates all local variables simultaneously instead of reusing stack slots for non-overlapping lifetimes
- May need stack slot reuse optimization or investigation of frame layout
