...
- [ ] **#3 — No system-default destination; bare `lp` / `lp file` with no `-d`/`LPDEST`/`PRINTER` always exits 1.** `print/lp.rs:54-73` (`get_destination` returns `Err("no destination specified")` when all three are absent). POSIX DESCRIPTION 103086–103087: "If −d is not specified, and neither the LPDEST nor PRINTER environment variable is set, an unspecified destination is used." RATIONALE 103180–103182 is emphatic: lp "should always be able to execute with no options or operands and send the standard input to an unspecified output device." This is *technically* permissible via 103059–103061 ("If such a device is not available … the lp utility shall exit with a non-zero exit status"), but it violates the documented intent. Fix: when no destination is given, consult a configured default IPP printer (e.g. the CUPS default) instead of hard-failing.
- [ ] **#4 — Destination must be an `ipp://` URI; historical printer-*name* values are rejected.** `print/lp.rs:76-84` (`validate_uri` requires the `ipp://` prefix). A user with the ordinary `PRINTER=myprinter` or `LPDEST=myprinter` (the exact System V / BSD values the spec's precedence machinery exists to support — RATIONALE 103195–103201) gets `invalid destination URI (must be ipp://...)` and exit 1. Permitted by 103084–103089 (destination names are unspecified/vary), but it defeats interop. Fix (architectural): resolve a bare destination *name* to a configured `ipp://` URI (name→URI map / config), or formally document the `ipp://` requirement as this implementation's destination-name format.

#### Minor

- [ ] **#5 — Multi-file run aborts on the first error; later operands are not attempted.** `print/lp.rs:206-224` — both `read_input` (`:215`) and `send_print_job` (`:218`) propagate with `?`, so `lp a b c` stops at the first failing file even though earlier files were already submitted. CONSEQUENCES OF ERRORS is "Default" (103159–103160), so this is *conforming*, but a per-file policy that records the failure, continues to siblings, and exits non-zero at the end matches historical lp and the EXIT STATUS wording ("0  All input files were processed successfully").
- [ ] **#6 — `-n copies` (`u32`) is cast to `i32` and wraps negative for values > 2147483647.** `print/lp.rs:139` (`IppValue::Integer(args.copies as i32)`); the clap validator is `range(1..)` on `u32` (`:31`), which accepts up to `u32::MAX`. `lp -n 3000000000` sends a negative IPP `copies`. Fix: validate `copies` against `i32::MAX` (or the IPP `copies` upper bound) at the clap layer.
...
- [ ] **#8 — `-o` options without `=` are silently discarded.** `print/lp.rs:145-160` (the `split_once('=')` `else` branch drops the option with no diagnostic — `:159`). `-o` semantics are spec-unspecified (103096–103098) so this is conforming, but a malformed-option diagnostic to stderr would aid users.
- [ ] **#9 — Request ID degrades to `<dest>-0` when the IPP response omits `job-id`.** `print/lp.rs:182-188` (`.unwrap_or(0)`). POSIX 103065 mandates a *unique* request ID per request; a `-0` fallback is non-unique. Only bites against a non-conformant server, hence Minor. Fix: treat a missing `job-id` as an error (exit non-zero) rather than fabricating `0`.
