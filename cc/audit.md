...
| C99 | Mostly CONFORMS (VLAs, designated initializers, compound literals, flexible array members, `restrict`, `inline`, `_Bool`, `long long`, `_Complex`, `__func__`, UCNs, hex floats). Gap: #L4 |
| C11 | PARTIAL — `_Static_assert`, `_Alignas`/`_Alignof`, `_Noreturn`, `_Thread_local`, anonymous struct/union, and the explicit atomic API all work. Gaps: #X1, #X3, #X5, #X6, #X7 |
...
- [ ] Trigraphs (#P11) — still uncovered. _(The bare `#` null directive, macro redefinition and arity diagnostics, large `#if` hex constants, and comment-as-whitespace stringification are all covered by `cc/tests/preprocessor/conformance.rs`, a suite that asserts on `-E` **text** rather than on a compiled program's exit code.)_
...
- [ ] `_Generic`, Unicode literal prefixes, `CMPLX` — still uncovered. _(Incompatible typedef redefinition is covered: `diagnostics_incompatible_typedef_redefinition_is_rejected`.)_
