...
- [ ] **#LS4 — 6-month date decision uses mtime even under `-c`/`-u`.** `tree/ls_util/entry.rs:741-758`. The displayed `<date and time>` is ctime/atime under `-c`/`-u`, but the recent-vs-old (`< 6 months`) branch always recomputes from `LastModificationTime`, so a file with old mtime but recent atime shows the wrong format under `-u`. Fix: base the recency test on the same timestamp being displayed.
