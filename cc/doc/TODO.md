...
| aarch64 `va_arg` of an over-aligned composite | Taken from the `__stack` path it is read at the wrong offset: c17 rounds the cursor up to 16 whenever the type's alignment is at least 16, and `__attribute__((aligned (32)))` is folded into a struct's `composite.align`, so an over-aligned HFA is rounded where gcc does not round for over-alignment at all. A plain HFA is correct, and so is the register-save-area path; only the stacked, over-aligned case diverges. Deriving the ABI alignment from the members rather than from `alignment()` is the likely fix, but the rule wants pinning against gcc for a naturally 16-aligned composite before it is written |
