...
fn format_time(timestamp: i64) -> String {
    if timestamp == 0 {
        ...
        let dt: DateTime<Local> = Local.timestamp_opt(timestamp, 0).unwrap();
...
fn read_macos_shm() -> Vec<ShmInfo> {
    ...
    // On macOS, we iterate through possible shmids
...
fn read_macos_sem() -> Vec<SemInfo> {
    ...
    // On macOS, we iterate through possible semids
    for semid in 0..4096 {
                ...
                for entry in entries {
                    ...
                    if args.creator {
                        ...
                        line.push_str(&format!(
                            " {:>8} {:>8}",
                            &creator[..creator.len().min(8)],
                            &cgroup[..cgroup.len().min(8)]
                        ));
                    }
                    if args.outstanding {
