...
### Bug R5: str.format() with keyword arguments — NOT YET FIXED
- `_B32_ENCODE_DOCSTRING.format(encoding='base32')` raises `IndexError: Replacement index 2833333305`
- Affects base64.py → ssl.py → asyncio → test framework startup
- Basic positional `str.format()` works; only keyword args fail
- 2833333305 = 0xA8E5A039 is consistent garbage — likely a codegen bug in `_PyUnicodeWriter` or `_PyUnicode_FormatAdvancedWriter`
- **PRIMARY BLOCKER for `make test`**
...
### Bug W: Timestamp overflow — NOT YET FIXED
- `OverflowError: timestamp out of range for platform time_t` in test worker threads
- Likely related to `time_t` / `struct timespec` handling in pcc-compiled code
- May be caused by 32-bit truncation of 64-bit time values
