...
/// [TestPlan] but for binary data on standard output
pub struct BinaryTestPlan {
    pub cmd: String,
    pub args: Vec<String>,
    pub stdin_data: String,
...
fn run_binary_test_base(plan: BinaryTestPlan) -> (BinaryTestPlan, Output) {
    let relpath = format!("target/release/{}", plan.cmd);
    let test_bin_path = std::env::current_dir()
        .unwrap()
        .parent()
...
pub fn run_binary_test(plan: BinaryTestPlan) {
    let (plan, output) = run_binary_test_base(plan);
