...
- [ ] **#C112 — `sizeof` of an incomplete array *expression* is accepted and answers 0.** **Open, Minor.** **[probed]** `extern int a[]; sizeof a` compiles and gives 0; gcc rejects it (6.5.3.4p1). #C90 closed the type-name form and recorded this one as "not covered", and #C111 did not reach it either: both work from the type, and the type cannot tell an incomplete array from a variably modified one -- `int[]`, `int[n]` and `int[m]` all intern to one `TypeId`. The expression form has no `dims` to consult the way `ExprKind::SizeofType` does. What it needs is the piece #C89's expression half leaned on: a record on the *symbol* of whether its array was declared with extents, which today lives only on the AST declarator (`InitDeclarator::vla_sizes`). Narrow enough to be worth a `bool` on `Symbol`; kept out of #C111 because that finding answers about *objects being defined* while this answers about an operand being measured.
